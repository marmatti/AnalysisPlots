...
def main():
    ...
    dope_files = sorted(glob("DecodedDOPEOneHelixRun1.csv"))
    ...
    decoded_scores_extended = result_df[result_df['Dataset'] == 'decoded']['DOPE score'].iloc[:200]
    mean_score_extended = np.mean(decoded_scores_extended)
    plt.hlines(y=mean_score_extended, xmin=result_df['time'].min(), xmax=200, ls='--', color='dimgrey')
    plt.scatter([result_df['time'].min(), 200], [mean_score_extended, mean_score_extended], color='dimgrey', zorder=5)  # Adding ticks at the extremes

    #insert line for average RMSD of compact conformation
    decoded_scores_compact = result_df[result_df['Dataset'] == 'decoded']['DOPE score'].iloc[200:]
    mean_score_compact = np.mean(decoded_scores_compact)
    plt.hlines(y=mean_score_compact, xmin=200, xmax=result_df['time'].max(), ls='--', color='lightgrey')
    plt.scatter([200, result_df['time'].max()], [mean_score_compact, mean_score_compact], color='lightgrey', zorder=5)  # Adding ticks at the extremes
    ...
    plt.savefig('DOPEVStOneHelix4Report.jpg', dpi=300, bbox_inches='tight')
