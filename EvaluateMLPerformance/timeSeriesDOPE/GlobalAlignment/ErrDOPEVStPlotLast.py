...
def main():
    ...
    dope_files = sorted(glob("DecodedDOPEProteinH*.csv"))
    ...
    result_df.to_csv('CheckingResult.csv')
    ...
    decoded_scores_extended = result_df[result_df['Dataset'] == 'Decoded']['Error'].iloc[:188]
    mean_score_extended = np.mean(decoded_scores_extended)
    plt.hlines(y=mean_score_extended, xmin=result_df['TimeFrame'].min(), xmax=188, ls='--', color='darkorange')
    plt.scatter([result_df['TimeFrame'].min(), 188], [mean_score_extended, mean_score_extended], color='darkorange', zorder=5)  # Adding ticks at the extremes

    #insert line for average RMSD of compact conformation
    decoded_scores_compact = result_df[result_df['Dataset'] == 'Decoded']['Error'].iloc[188:]
    mean_score_compact = np.mean(decoded_scores_compact)
    plt.hlines(y=mean_score_compact, xmin=188, xmax=result_df['TimeFrame'].max(), ls='--', color='navajowhite')
    plt.scatter([188, result_df['TimeFrame'].max()], [mean_score_compact, mean_score_compact], color='navajowhite', zorder=5)  # Adding ticks at the extremes
    ...
    plt.savefig('StandardErrDOPEVStProteinH4Report.jpg', dpi=300, bbox_inches='tight')
